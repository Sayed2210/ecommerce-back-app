...
export class AddReturnsAndNewsletterEntities1776035622526 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (!statusEnumExists.length) {
            await queryRunner.query(`CREATE TYPE "public"."return_requests_status_enum" AS ENUM('pending', 'approved', 'rejected', 'refunded')`);
        }
        ...
        if (!reasonEnumExists.length) {
            await queryRunner.query(`CREATE TYPE "public"."return_requests_reason_enum" AS ENUM('defective', 'wrong_item', 'not_as_described', 'changed_mind')`);
        }
        ...
        if (!fkOrderItemExists.length) {
            await queryRunner.query(`ALTER TABLE "return_requests" ADD CONSTRAINT "FK_return_requests_order_item_id" FOREIGN KEY ("order_item_id") REFERENCES "order_items"("id") ON DELETE CASCADE ON UPDATE NO ACTION`);
        }
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        // Drop foreign keys
        await queryRunner.query(`ALTER TABLE "return_requests" DROP CONSTRAINT IF EXISTS "FK_return_requests_order_item_id"`);
        await queryRunner.query(`ALTER TABLE "return_requests" DROP CONSTRAINT IF EXISTS "FK_return_requests_order_id"`);
        await queryRunner.query(`ALTER TABLE "return_requests" DROP CONSTRAINT IF EXISTS "FK_return_requests_user_id"`);
        ...
        // Drop enums
        await queryRunner.query(`DROP TYPE IF EXISTS "public"."return_requests_reason_enum"`);
