...
export class MailerService {
  ...
  async sendEmail(options: EmailOptions): Promise<void> {
    try {
      ...
      this.logger.log(
        `✉️  Email sent successfully to ${Array.isArray(options.to) ? options.to.join(', ') : options.to} (Message ID: ${result.messageId})`,
      );
  ...
  async sendEmailAsync(options: EmailOptions): Promise<void> {
    ...
    this.logger.log(
      `📨 Email queued for ${Array.isArray(options.to) ? options.to.join(', ') : options.to}`,
    );
  }
  ...
  async sendAbandonedCartFollowUp(
    to: string,
    data: {
      userName: string;
      cartItems: any[];
