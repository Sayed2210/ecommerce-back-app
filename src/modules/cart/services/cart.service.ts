...
export class CartService {
  ...
  async addItem(cartId: string, itemDto: AddCartItemDto): Promise<Cart> {
    ...
    const existingItem = cart.items.find(
      (item) =>
        item.product.id === itemDto.productId &&
        item.variant?.id === itemDto.variantId,
    );
    ...
    if (updatedCart.user?.id) {
      this.notificationsGateway.sendCartUpdate(
        updatedCart.user.id,
        updatedCart,
      );
    }
  ...
  async clearCart(cartId: string) {
    ...
    if (cart.user?.id) {
      this.notificationsGateway.sendCartUpdate(cart.user.id, {
        ...cart,
        items: [],
        subtotal: 0,
      });
