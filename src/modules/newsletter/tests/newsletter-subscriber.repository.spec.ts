...
describe('NewsletterSubscriberRepository', () => {
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: getRepositoryToken(NewsletterSubscriber), useValue: mockRepo },
                { provide: DataSource, useValue: mockDataSource },
            ],
        }).compile();

        repository = module.get<NewsletterSubscriberRepository>(NewsletterSubscriberRepository);
    });
    ...
    describe('save', () => {
        it('delegates to the underlying TypeORM repository', async () => {
            const entity = { id: 's1', email: 'test@example.com' } as NewsletterSubscriber;
