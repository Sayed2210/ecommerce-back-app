...
const makeConfigService = (values: Record<string, string>) =>
    ({ get: jest.fn((key: string, fallback?: string) => values[key] ?? fallback) } as any);
    ...
    describe('with default rate only', () => {
        ...
        beforeEach(async () => {
            const module: TestingModule = await Test.createTestingModule({
                providers: [
                    TaxService,
                    { provide: ConfigService, useValue: makeConfigService({ DEFAULT_TAX_RATE: '0.1' }) },
                ],
            }).compile();
        ...
        beforeEach(async () => {
            const module: TestingModule = await Test.createTestingModule({
                providers: [
                    ...
                    {
                        provide: ConfigService,
                        useValue: makeConfigService({ DEFAULT_TAX_RATE: '0.1', TAX_RATES: 'not-json' }),
                    },
                ],
            }).compile();
            service = module.get<TaxService>(TaxService);
