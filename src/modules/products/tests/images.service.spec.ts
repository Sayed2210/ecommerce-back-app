...
const mockS3 = {
  ...
  uploadFile: jest
    .fn()
    .mockResolvedValue(
      'https://bucket.s3.us-east-1.amazonaws.com/products/prod-1/uuid.jpg',
    ),
...
describe('ProductImagesService', () => {
  ...
  describe('upload', () => {
    it('throws 404 when product does not exist', async () => {
      ...
      await expect(service.upload('prod-1', makeFile(), {})).rejects.toThrow(
        NotFoundException,
      );
  ...
  describe('remove', () => {
    ...
    it('promotes next image to primary when primary is deleted', async () => {
      ...
      expect(mockImageRepo.update).toHaveBeenCalledWith('img-2', {
        isPrimary: true,
      });
    });
  });
  ...
  describe('findAll', () => {
    it('throws 404 when product does not exist', async () => {
      ...
      await expect(service.findAll('prod-1')).rejects.toThrow(
        NotFoundException,
      );
    });
