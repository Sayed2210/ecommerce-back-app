...
describe('ReturnsService', () => {
    ...
    describe('create', () => {
        const dto = { orderId: 'order-1', orderItemId: 'item-1', reason: 'Damaged' };
        ...
        it('throws ForbiddenException when order belongs to another user', async () => {
            orderRepository.findOne.mockResolvedValue(makeOrder({ user: { id: 'other-user' } as any }));
        ...
        it('throws BadRequestException when order is not delivered', async () => {
            orderRepository.findOne.mockResolvedValue(makeOrder({ status: OrderStatus.PENDING }));
        ...
        it('throws BadRequestException when return already exists for item', async () => {
            orderRepository.findOne.mockResolvedValue(makeOrder());
            orderItemRepository.findOne.mockResolvedValue({ id: 'item-1', unitPrice: '25', quantity: 2 });
    ...
    describe('findOne', () => {
        ...
        it('throws ForbiddenException when userId does not match', async () => {
            returnRepository.findOneWithOptions.mockResolvedValue(makeReturn({ user: { id: 'other' } as any }));

            await expect(service.findOne('ret-1', 'user-1')).rejects.toThrow(ForbiddenException);
        });
    });
    ...
    describe('process', () => {
        ...
        it('calls Stripe refund and marks order as REFUNDED on approval with paymentIntentId', async () => {
            ...
            const saved = makeReturn({ status: ReturnStatus.REFUNDED, refundId: 'ref_123' });
            returnRepository.save.mockResolvedValue(saved);
        ...
        it('approves without refund when no paymentIntentId', async () => {
            returnRepository.findOneWithOptions.mockResolvedValue(makeReturn());
            orderRepository.findOne.mockResolvedValue(makeOrder({ paymentIntentId: null }));
